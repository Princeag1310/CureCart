            ...
            {isAdding && (
              <tr className="bg-blue-50/50">
                ...
                <td className="px-4 py-4 text-right space-x-2">
                  <button onClick={handleAddSubmit} aria-label="Save medicine" className="p-2 text-white bg-green-500 hover:bg-green-600 rounded-md"><Save className="w-4 h-4"/></button>
                  <button onClick={() => setIsAdding(false)} aria-label="Cancel" className="p-2 text-gray-500 bg-gray-200 hover:bg-gray-300 rounded-md"><X className="w-4 h-4" /></button>
                </td>
              </tr>
            )}
            ...
            {filteredMedicines.map((med) => (
              <tr key={med.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="px-4 py-4 text-right space-x-2 whitespace-nowrap">
                  {editingId === med.id ? (
                    <>
                      <button onClick={handleSaveEdit} aria-label="Save changes" className="p-2 text-white bg-blue-600 hover:bg-blue-700 rounded-md transition-colors"><Save className="w-4 h-4" /></button>
                      <button onClick={handleCancelEdit} aria-label="Cancel editing" className="p-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors"><X className="w-4 h-4" /></button>
                    </>
                  ) : (
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleEditClick(med)} aria-label="Edit medicine" className="p-2 text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Pen className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(med.id)} aria-label="Delete medicine" className="p-2 text-red-600 hover:bg-red-50 rounded-md transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  )}
                </td>
              </tr>
